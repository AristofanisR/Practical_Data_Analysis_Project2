...
```{r}
# 1st Method: Perform KNN imputation with 5 neighbors
data_imp <- kNN(data, k = 5, imp_var = FALSE)

```
$K-$Nearest Neighbors (KNN) Imputation is a method that fills in missing values in a dataset by looking at the values of other, similar observations. For each missing value in the dataset, the algorithm identifies the $k$ (=5 here) nearest neighbors (observations) based on other variables that are complete. These "neighbors" are the most similar observations in terms of overall feature values. Once the nearest neighbors are identified, the missing value is filled in by taking the average (for numerical data) or the most frequent category (for categorical data) of the values from these neighbors. Unlike simpler methods like mean or median imputation, KNN considers patterns and relationships within the data, making it more likely to produce values that are consistent with the existing data structure. This can be especially helpful if there are clusters or groups in the data, as KNN captures these structures.(?????? are there any clusters?)

```{r}
# Second Method: MICE 
# Perform MICE imputation
data_mice <- mice(data, m = 5
                  , method = "pmm", maxit = 50, seed = 1, printFlag= FALSE)

# Complete the data by extracting one of the imputed datasets
data_mice <- complete(data_mice, action = 1)
```
...
# Plot ROC curve with ggplot2
ROC_logistic = ggplot(roc_data, aes(x = 1-Specificity, y = Sensitivity)) +
  geom_line(color = "purple", size = 1) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey") +
  annotate("text", x = 0.8, y = 0.2, label = paste("AUC =", round(auc_value, 2)), size = 5, color = "Black") +
  labs(
    title = "ROC for the Stepwise Logistic Model",
    x = "1 - Specificity",
    y = "Sensitivity"
  ) +
  theme_minimal() +
...
calib_error_bar = ggplot(calib_data) +
  ...
  labs(x = "Expected Probability of Smoking Abstinence",
       y = "Actual Smoking Abstinence",
       title = "Calibration Plot for Logistic Model with Error Bars") +
  theme_minimal()
...
calib_loess = ggplot(calib_data, aes(x = predicted, y = observed)) +
  ...
  labs(x = "Predicted Probability of Smoking Abstinence",
       y = "Actual Smoking Abstinence",
       title = "Calibration Plot for Logistic Model using LOESS") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))


print(calib_error_bar + calib_loess + ROC_logistic)
```

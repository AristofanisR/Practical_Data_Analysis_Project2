...
```{r}
# Mosaic between edu, income
mosaic(~ inc + edu_merged, data = data_mice, 
       shade = TRUE,           # Apply shading based on chi-squared statistic
       legend = TRUE,         # Show a legend
       labeling = labeling_values,   # Show frequencies on tiles
       gp = gpar(fontsize = 12))  # Adjust the size of text labels


# Plot between readiness, NMR
...
calib_loess_lasso = ggplot(calib_data, aes(x = predicted, y = observed)) +
  ...
  geom_line(aes(y = loess_pred), color = "forestgreen", linetype = "dashed") +  

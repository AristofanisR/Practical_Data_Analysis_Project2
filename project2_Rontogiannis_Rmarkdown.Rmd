...
vcd::mosaic(~ inc + edu_merged, data = data_mice, shade = TRUE, legend = TRUE, labeling = labeling_values) 


# Heatmap between race, education
ggplot(data = data_mice) +
...
ggplot(data_mice, aes(x = sex_ps, y = NMR, color = sex_ps)) +
  geom_beeswarm(size = 2, alpha = 0.7) +
  labs(x = "Sex", y = "NMR", title = "Beeswarm Plot") +
  theme_minimal() +
  theme(legend.position = "none") +
  scale_fill_manual(values = c("slateblue3", "palevioletred"))



# Raincloud plot between edu, income
ggplot(data_mice, aes(x = race, y = ftcd_score, fill = race)) +
...
We can observe that except BA1 and Var1 (obviously - we forced them) there are no other common variables between all three model selection methods. However, there are two common variables between Lasso and Stepwise Regression (ftcd_score, NMR). ########## den ecw tipota allo koino####moderators den ecw katholou ######### episis na pw gia odds ratio
...
ROC_lasso = ggplot(roc_data, aes(x = 1-Specificity, y = Sensitivity)) +
  geom_line(color = "mediumpurple2", size = 1) +
...
calib_loess_lasso = ggplot(calib_data, aes(x = predicted, y = observed)) +
  ...
  geom_line(aes(y = loess_pred), color = "mediumpurple2", linetype = "dashed") +  
...
kable(full_df, row.names = FALSE, 
      col.names = c("Variables","Lasso", "Stepwise", "Best Subset",
                     "Lasso OR", "Stepwise OR", "Best Subset OR"),
        caption = "Summary of Coefficients and Odds Ratios across 3 Selection Methods (for Predictors)") 
